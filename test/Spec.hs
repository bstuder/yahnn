...
testNetwork =
    T.describe "Test of network functions:" $ do
        ...
        T.it "Computes a gradient for a given target" $ do
            ...
            (forwardResult >>= \justForwardResult -> N.backward network justForwardResult vector L.MSE) `T.shouldSatisfy`
                \backwardResult -> False -- FIXME
